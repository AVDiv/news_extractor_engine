...
from news_extractor_engine.utils.checksum_validator import generate_checksum, check_or_create_checksum
...
def test_incorrect_checksum_file_renewal():
  ...
  if os.path.exists(FAKE_CHECKSUM_FILE_PATH):
    with open(FAKE_CHECKSUM_FILE_PATH, 'w') as checksum_file:
      checksum_file.write(hashlib.sha256("fake_checksum".encode('utf-8')).hexdigest())
