...
class Engine:
    ...
    async def __run_feed_sync_cycle(self, feed: FeedReader):
    ...
    async def generate_feed_sync_tasks(self, feed: FeedReader):
        task = asyncio.create_task(
            self.__run_feed_sync_cycle(feed), name=feed.source.name
        )
