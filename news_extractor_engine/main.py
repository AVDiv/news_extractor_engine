  ...
  while True:
    ...
    try:
      ...
      if feed_data.has_updated_since_last_request:
        discord.send_embed(
          title=f"New article from {feed.source.name}",
          description=f"**{feed_data.feed['entries'][0]['title']}**\n{feed_data.feed['entries'][0]['summary']}",
          url=str(feed_data.feed['entries'][0]['link']),
          color=0xddcfee
        )
    except Exception as e:
      logging.error(e)
      discord.send_error(e)
      return
...
async def main():
  ...
  with dev_mode():
    feed_check_list = []
    print(news_sources[1:10])
  for i, row in enumerate(news_sources[1:10].iter_rows()):
    source = ArticleSource(
      id=i,
      name=row[2],
      domain=row[0],
      rss_url=row[1],
      categories=list(row[3])
    )
